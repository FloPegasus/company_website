...
Class Content extends DBConnection {
	...
	public function service(){
		...
		foreach($_POST as $k => $v){
			if(!in_array($k,array('id','description', 'description_en','old_file'))){
				if(!empty($data)) $data .= ", ";
				$data .= "`$k` = '$v'";
			}
			if ($k == 'description') {
                if(!empty($data)) $data .= ", ";
                $data .= "`description` = '".addslashes(htmlentities($v))."'";
            } elseif ($k == 'description_en') {
                if(!empty($data)) $data .= ", ";
                $data .= "`description_en` = '".addslashes(htmlentities($v))."'";
            }
		}
	...
	public function testimonial(){
		...
		$data = "";
		foreach($_POST as $k => $v){
			if(!in_array($k,array('id','message','message_en','old_file'))){
				if(!empty($data)) $data .= ", ";
				$data .= "`$k` = '$v'";
			} elseif ($k == 'message') {
                if(!empty($data)) $data .= ", ";
                $data .= "`message` = '".addslashes(htmlentities($v))."'";
            } elseif ($k == 'message_en') {
                if(!empty($data)) $data .= ", ";
                $data .= "`message_en` = '".addslashes(htmlentities($v))."'";
            }
		}
